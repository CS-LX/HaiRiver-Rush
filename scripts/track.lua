...
-- 瓦片可见性流式常量
local TILE_AHEAD   = 55          -- 前方启用 55 个瓦片（~550m）
local TILE_BEHIND  = 15          -- 后方保留 15 个瓦片（~150m）
local prevVisIdx   = -1          -- 上次更新可见性时的 currentIdx，避免每帧重算
...
local function BakeLoop()
    ...
    for i = 1, LOOP_N do
        ...
        -- 先设位置（SetEnabled(false) 仍保持，稍后由 UpdateTileVisibility 统一开启）
        tile:SetPosition(Vector3(midX, -0.05, midZ))
        tile:SetRotation(Quaternion(0, n.heading, 0))
        -- 保持 disabled，由 UpdateTileVisibility 按需启用
...
-- ─────────────────────────────────────────────────────────────
--  内部：按 currentIdx 启用/禁用瓦片（仅 index 变化时才执行）
-- ─────────────────────────────────────────────────────────────
local function UpdateTileVisibility()
    if LOOP_N == 0 then return end
    if currentIdx == prevVisIdx then return end   -- index 未变，跳过
    prevVisIdx = currentIdx

    for i = 1, LOOP_N do
        local fwd = (i - currentIdx + LOOP_N) % LOOP_N
        local active = (fwd <= TILE_AHEAD) or (fwd >= LOOP_N - TILE_BEHIND)
        loopTiles[i]:SetEnabled(active)
    end
end
